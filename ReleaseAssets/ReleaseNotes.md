...
Number of cycles now scales to match the (best-case) minimum required to deviate one count (only kicks-in when convergence gets very, very precise). Use this in conjunction with the nudge hotkeys to verify whether observed deviations exceed margin of error (residual angle artifact can deviate up to half count both ways).
