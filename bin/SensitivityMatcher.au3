...
Func HandyCalculator($idGUICalc, ByRef $sInput, $idMsg)
   ...
   Else
      If $idGUICalc == "INITIALIZE" Then
         ...
         $sInput[0]=GUICtrlCreateInput($gSens             , 85, 10, 80, 20)
         ...
         GUICtrlCreateLabel("Virtual Scale:",10,13,75,15,$SS_RIGHT)
         GUICtrlCreateLabel("Physical Scale:",10,33,75,15,$SS_RIGHT)
         GUICtrlCreateLabel("deg",170,13,35,15,$SS_LEFT)
         ...
         Local $hInc    = GUICtrlGetHandle($sInput[0])
                         _GUIToolTip_AddTool($hToolTip, 0, "Degree per virtual count", $hInc)
         Local $hCPI    = GUICtrlGetHandle($sInput[1])
                         _GUIToolTip_AddTool($hToolTip, 0, "Virtual count per Inch", $hCPI)
         Local $hDgm    = GUICtrlGetHandle($sInput[2])
                         _GUIToolTip_AddTool($hToolTip, 0, "Degree Per Millimeter", $hDgm)
         Local $hMPI    = GUICtrlGetHandle($sInput[3])
                         _GUIToolTip_AddTool($hToolTip, 0, "Minute (of arc) Per Inch", $hMPI)
         Local $hCcm    = GUICtrlGetHandle($sInput[4])
                         _GUIToolTip_AddTool($hToolTip, 0, "Centimeter Per Revolution", $hCcm)
         Local $hCin    = GUICtrlGetHandle($sInput[5])
                         _GUIToolTip_AddTool($hToolTip, 0, "Inch Per Revolution", $hCin)
         For $i = 0 to 5 
